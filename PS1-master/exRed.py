...
#src = np.copy(img_r)	# rose is source image
#dst = np.copy(img_d)	# dofin is destination image
src = img_r
dst = img_d
...
y_src = img_r.shape[0]	# height of src image
x_src = img_r.shape[1]	# width

cs1 = int(x_src/2 - 100/2)
cs2 = int(x_src/2 + 100/2)
rs1 = int(y_src/2 - 100/2)
rs2 = int(y_src/2 + 100/2)
...
cd1 = int(x_dst/2 - 100/2)
cd2 = int(x_dst/2 + 100/2)
rd1 = int(y_dst/2 - 100/2)
rd2 = int(y_dst/2 + 100/2)
